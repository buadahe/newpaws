 ...
 * @since 1.2
 */
function mgm_bp_tpack_theme_setup() {
	global $bp;
...
add_action( 'after_setup_theme', 'mgm_bp_tpack_theme_setup', 11 );
 ...
 * @since 1.2
 */
function mgm_bp_tpack_enqueue_scripts() {
	...
	/* @since MGM 1.2 to fix BuddyPress 2.1 JS Bug. */
	...
	/* END @since MGM 1.2 to fix BuddyPress 2.1 JS Bug. */
...
add_action( 'wp_enqueue_scripts', 'mgm_bp_tpack_enqueue_scripts' );
 ...
 * @since 1.2
 */
function mgm_bp_tpack_enqueue_styles() {
...
add_action( 'wp_enqueue_scripts', 'mgm_bp_tpack_enqueue_styles' );
 ...
 * @since 1.2
 */
function mgm_bp_tpack_use_wplogin() {
	// returning 2 will automatically use wp-login
	return 2;
}
add_filter( 'bp_no_access_mode', 'mgm_bp_tpack_use_wplogin' );
endif;
 ...
 * @since 1.2
 */
function mgm_bp_tpack_activity_secondary_avatars( $action, $activity ) {
...
add_filter( 'bp_get_activity_action_pre_meta', 'mgm_bp_tpack_activity_secondary_avatars', 10, 2 );
...
if ( version_compare( BP_VERSION, '1.3', '<' ) ) :
	...
	function mgm_bp_tpack_wp_pages_filter( $page_html ) {
	...
	add_filter( 'wp_dropdown_pages', 'mgm_bp_tpack_wp_pages_filter' );

	/* Hijack the saving of page on front setting to save the activity stream setting */
	function mgm_bp_tpack_page_on_front_update( $oldvalue, $newvalue ) {
	...
	add_action( 'pre_update_option_page_on_front', 'mgm_bp_tpack_page_on_front_update', 10, 2 );

	/* Load the activity stream template if settings allow */
	function mgm_bp_tpack_page_on_front_template( $template ) {
		global $wp_query;
	...
	add_filter( 'page_template', 'mgm_bp_tpack_page_on_front_template' );
	...
	function mgm_bp_tpack_fix_get_posts_on_activity_front() {
		global $wp_query;

		if ( !empty($wp_query->query_vars['page_id']) && 'activity' == $wp_query->query_vars['page_id'] )
			$wp_query->query_vars['page_id'] = '"activity"';
	}
	add_action( 'pre_get_posts', 'mgm_bp_tpack_fix_get_posts_on_activity_front' );
...
function mgm_bp_notification_badge() {
 	...
 	if ( is_user_logged_in() ) {
	 	...
	 	$notifications = bp_core_get_notifications_for_user( bp_loggedin_user_id() );
	        ...
	        if ( $notifications ) {
	            ...
	            echo '<span class="mgm-bp-badge">'.$badge.'</span>';
	        }
...
/*
==========================================================
BBPRESS BREADCRUMBS EDITS
==========================================================
*/


function mgm_bbp_breadcrumb() {
    $args['sep'] = '<span class="glyphicon glyphicon-chevron-right"></span>';
	$args['home_text'] = __('Home','mightymag');
    return $args;
}

add_filter('bbp_before_get_breadcrumb_parse_args', 'mgm_bbp_breadcrumb' );


/*
==========================================================
BBPRESS 2.5.4 WP 4 COMPATIBILITY FIX @since MGM 1.3
==========================================================
*/

//moved to admin/functions-extended/fn-bbpress.php @since MGM 1.4
