    ...
    function loading_page_init(){
        if(!is_admin()){
            ...
            if($op &&  $op['enabled_loading_screen'])
			{
				...
				if( !empty($op['codeBlock']) )
					add_filter( 'the_content', 'loadin_page_add_codeBlock' );
			}    
        }
...
if(!function_exists('loadin_page_add_codeBlock')){
	function loadin_page_add_codeBlock( $the_content )
	{
		global $loadin_page_codeBlock_added;
		$op = get_option('loading_page_options');	
		if( !empty($op['codeBlock']) && empty($loadin_page_codeBlock_added) )
		{
			$loadin_page_codeBlock_added = 1;
			$the_content = '<div id="loadin_page_codeBlock">'.$op['codeBlock'].'</div>'.$the_content;
		}	
		return $the_content;
	}
}
...
if(!function_exists('loading_page_admin_resources')){
    function loading_page_admin_resources($hook){
        if(strpos($hook, "loading-page") !== false){
            ...
            wp_enqueue_script('lp-admin-script', LOADING_PAGE_PLUGIN_URL.'/js/loading-page-admin.js', array('jquery', 'thickbox', 'farbtastic'), 'free-1.0.17');
        }
...
if(!function_exists('loading_page_enqueue_scripts')){
    function loading_page_enqueue_scripts(){
        ...
        $op = get_option('loading_page_options');
        wp_enqueue_style('codepeople-loading-page-style', LOADING_PAGE_PLUGIN_URL.'/css/loading-page'.(($op['pageEffect'] != 'none') ? '-'.$op['pageEffect'] : '').'.css', array(), 'free-1.0.17');
        ...
        if( $loadingScreen ){
            ...
            if($s){
                if(!empty($s['style'])){
                    wp_enqueue_style('codepeople-loading-page-style-'.$s['id'], $s['style'], array(), 'free-1.0.17');
                }
                
                if(!empty($s['script'])){
                    wp_enqueue_script('codepeople-loading-page-script-'.$s['id'], $s['script'], array('jquery'), 'free-1.0.17');
                    $required[] = 'codepeople-loading-page-script-'.$s['id'];
                }
            }    
            wp_enqueue_script('codepeople-loading-page-script', LOADING_PAGE_PLUGIN_URL.'/js/loading-page.js', $required, 'free-1.0.17');
                    ...
                    <div class="inside">
                        ...
                        <table class="form-table">
                            ...
                            <tr>
                                ...
                                <td>
									<input type="text" name="lp_additionalSeconds" id="lp_additionalSeconds" value="<?php print(esc_attr($loading_page_options['additionalSeconds'])); ?>" />
                            ...
                            <tr>
                                ...
                                <td>
									<textarea name="lp_codeBlock" id="lp_codeBlock" rows="6" style="width:80%;"><?php print(esc_textarea($loading_page_options['codeBlock'])); ?></textarea>
								</td>
                            </tr>
                            <tr>
