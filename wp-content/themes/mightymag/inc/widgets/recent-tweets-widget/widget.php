				...
				if ( ! empty( $title ) ){ echo $before_title . $title . $after_title; }
					...
					if(!empty($tp_twitter_plugin_tweets) && is_array($tp_twitter_plugin_tweets)){
						...
						<div class="tweet">
							...
							foreach($tp_twitter_plugin_tweets as $tweet){					
								if(!empty($tweet['text'])){
									...
									print '<li><span class="tweet_text">'.tp_convert_links($tweet['text']).'</span><br /><a class="tweet_time" target="_blank" href="http://twitter.com/'.$instance['username'].'/statuses/'.$tweet['status_id'].'">'.tp_relative_time($tweet['created_at']).'</a></li>';
						...
						print '
							...
							<a href="http://twitter.com/'.$instance['username'].'/statuses/'.$tweet['status_id'].'" class="twtr-join-conv btn btn-success">' . __('Join the Conversation', 'mightymag') . '</a>
						</div>';
					}else{
						print '
