...
class mgm_CodePeopleSearchInPlace {
		
	
	private $text_domain = 'mightymag';
	...
	*/
	function init(){
		// I18n
		//load_plugin_textdomain($this->text_domain, false, get_template_directory_uri(). '/inc/live-search/languages/');
		$this->javascriptVariables = array(
									...
									'root'	 => get_site_url(),
									'home'	 => home_url() /* djwd get_option(home) -> home_url */
							);
							
		// Fake variables to allow the translation for Poedit application
		$a = __('', $this->text_domain); 		
		$a = __('', $this->text_domain); 		
		$a = __('', $this->text_domain);
	} // End init
	...
	*/
    public function populate() {
		...
		$counter = 0;
		$limit = 4; // Number of results to display
		$post_list = array();
        ...
        foreach($posts as $result){
			...
			// Magma Variables
			$mgm_search_elements = of_get_option('mgm_search_elements');
			
			
			// Include the author in search results
			if ($mgm_search_elements['post_author'] == true ){
			...
			if ($mgm_search_elements['post_thumb'] == true ){
                if($result->post_type == 'attachment'){
                    ...
                    if ( function_exists('has_post_thumbnail') && has_post_thumbnail($result->ID) ) {
						...
						$src = wp_get_attachment_image_src( get_post_thumbnail_id( $result->ID ), 'menu-thumbs', false, '' ); /* djwd ultrafix */
						$obj->thumbnail = $src[0];
			...
			if ($mgm_search_elements['post_excerpt'] == true ){
				$length = of_get_option('mgm_search_charscount');
			...
			if($mgm_search_elements['post_date'] == true ){
				$obj->date = date(get_option('date_format'));;
			}	
...
/*		if(!is_admin() && !mgm_is_really_woocommerce() && $query->is_search){
            $query->set('post_type', array('post', 'page', 'product'));
            $query->set('post_status', array('publish', 'inherit'));
        }*/
		
		//djwd Commented out. While it allows product search to respond fine, breaks bbPress permalinks.
